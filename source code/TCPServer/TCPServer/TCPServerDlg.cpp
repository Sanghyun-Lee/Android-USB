...
BEGIN_MESSAGE_MAP(CTCPServerDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_MESSAGE(UM_ACCEPT_MESSAGE, &CTCPServerDlg::OnAcceptClient)
	ON_MESSAGE(UM_RECEIVE_MESSAGE, &CTCPServerDlg::OnReceiveData)
	ON_MESSAGE(UM_CLOSE_MESSAGE, &CTCPServerDlg::OnCloseSocket)
...
LRESULT CTCPServerDlg::OnReceiveData(WPARAM wParam, LPARAM lParam)
{
	char Rcvdata[1024];
...
void CTCPServerDlg::AddMessage(CString strMsg)
{
	UpdateData();
...
LRESULT CTCPServerDlg::OnCloseSocket(WPARAM wParam, LPARAM lParam)
{
	CDataSocket	*pDataSocket = (CDataSocket*)wParam;
	pDataSocket->Close();

	AddMessage("클라이언트가 접속을 종료했습니다.");
