...
///*
		//m++;
		//if(m <15 ) {
		//info("stub_send_ret_submit =%d=", m);
		info("========================");
		//info("type:%d", urb->type);
		//info("endpoint:0x%02x", urb->endpoint);
		//info("stauts:%d", urb->status);
		//info("flags:0x%08x", urb->flags);
		//info("buffer:0x%p", urb->buffer);
...
gboolean process_device_urb_test(GIOChannel *gio, GIOCondition condition, gpointer data)
{
	...
	if (condition & G_IO_IN) {
		fd = g_io_channel_unix_get_fd(gio);
		info("process_device_urb_test / sock : %d", fd);
